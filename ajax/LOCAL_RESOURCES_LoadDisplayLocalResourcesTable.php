...
$BUTTONS = new BUTTONS();
$LOCAL_RESOURCES = new LOCAL_RESOURCES($UTILITIES, $BUTTONS);

$authorization_key = htmlspecialchars($_REQUEST["authorization_key"]);

if($authorization_key === $_SESSION["key"]) {
    $partitions_id_type = $LOCAL_RESOURCES->UTILITIES_LOCAL_RESOURCES->GetPartitionIdsAndTypes();
    $string = $LOCAL_RESOURCES->DisplayLocalResourcesChildNodeRefreshed("w3-twothird", $partitions_id_type);
    echo $string;
//    echo "";
}
